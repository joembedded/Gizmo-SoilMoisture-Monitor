 ...
 * can be used standalone or as a modem (possibly both).
 *
 * Configuration in ltx_rak3172_user.h:
 * ====================================
 * - Define USAGE
 * - If LED is present: define LED
 * - Set device parameters (energy scheme, parameters, etc.)
 * - Define modem:
 *     #define STD_ENERGY_RAK3172LSIP_3V3
 *     or
 *     #define STD_ENERGY_RAK3172MODULE_3V3
 *
 * Additional commands for RUI3 (all via 'ATC+LTX'):
 * =================================================
 * Always available:
 * -----------------
 ...
 * --------------------------
 *  send        - 'port, hex..' sends frame 'smart' (confirmation only if needed when CFM=0)
 *
 * Only for USAGE_EXT_ATX:
 * -----------------------
 *  This mode extends the RAK RUI3 with additional commands, e.g.:
 *  - Incoming frames can be stored (no need to wait for "+EVT_RECEIVED" message)
 *  - Energy consumption of the action can be queried
 *  - Commands can also be sent in plain text from the server
 *
 *  write        - Saves setup/coefficients after changes in NVM
 *  factoryreset - Deletes all user data
 *  cmd=         - Sets the measurement command
 *  p=           - Sets measurement period in seconds
 *  hkr=         - Sets HK counter, after HK hkr data is transmitted without HK (saves energy)
 *  profile=     - 1–223 (measurement values as F32), 1001–1223 (measurement values as F16)
 *  wd=          - 0/1 for false/true: use watchdog
 *  k            - Set coefficients for calculation of measurement values
...
static int16_t send_txpayload(void)
{
  ...
  bool sres;
  if (mlora_info.txframe.tx_with_conf)
...
static int16_t parse_ltx_cmd(char *pc)
{
  ...
  else if (str_cmatch("send ", pc))
  {
    ...
    if (port < 1 || port > 223)
      ...
      res = -2013; // ':' not possible ( == new argument)
    else
    {
      pc++;
